 ...
 * @author al199
 */
public class BusAGui extends Form{
    
    private Resources theme;
    private BusService bs= new BusService();
    ArrayList<Bus> listBus= new ArrayList<>();

    public BusAGui(String title, Resources theme){
    ...
    private void addGUIs(){
        ...
        for (Bus b : listBus){
            ...
            del.addActionListener(event -> {
                bs.deleteBus(lbId1.getText());
                Dialog.show("Suppression avec success !", "le bus "+lbFab1.getText()+" a été supprimer avec success",null, "ok");
                System.out.println(lbFab1.getText());
            });
            item.add(btns);
            
            up.addActionListener(event ->{
                Bus bu= new Bus(Integer.parseInt(lbId1.getText()),lbFab1.getText(), lbMod1.getText(), Integer.parseInt(lbPlace1.getText()), Boolean.parseBoolean(lbPanne1.getText()));
                new UpdateBusGui("Update Bus", theme,bu).show();
            });
    ...
    public BusAGui(com.codename1.ui.util.Resources resourceObjectInstance) {
